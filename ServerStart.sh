...
eula_gen(){
    ...
    export answer="No"
    echo ""
    read -p "Type Yes is agree otherwise type No? " answer
...
install_server(){
    ...
    installer=($(ls forge*$MC_SERVER_MCVER*$MC_SERVER_FORGEVER*installer.jar 2>>logs/serverstart.log))
    if [[ $? == 0 ]]; then
        echo "DEBUG: Found forge jar: ${installer[0]}" >>logs/serverstart.log
        export answer="y"
        ...
        if [[ "$answer" =~ ^([yY][eE][sS]|[yY])+$ ]]; then
            ...
            mkdir -p DELETEME
            mv -f "./${installer[0]}" ./DELETEME >>logs/serverstart.log 2>&1
            installer=0
        fi
    else
        installer=0
    ...
    installer=($(ls forge*$MC_SERVER_MCVER*$MC_SERVER_FORGEVER*installer.jar 2>>logs/serverstart.log))
    if [[ $? -eq 0 ]] ; then
        export MC_SERVER_INSTALLER=${installer[0]}
        echo "Moving unneeded files/folders to ./DELETEME"
        echo "INFO: Moving unneeded files/folders to ./DELETEME" >>logs/serverstart.log 2>&1
        rm -rf ./DELETEME >>logs/serverstart.log 2>&1
        mv -f ./asm ./DELETEME >>logs/serverstart.log 2>&1
        mv -f ./libraries ./DELETEME >>logs/serverstart.log 2>&1
        mv -f ./library ./DELETEME >>logs/serverstart.log 2>&1
        mv -f ./minecraft_server*.jar ./DELETEME >>logs/serverstart.log 2>&1
        mv -f ./forge*universal.jar ./DELETEME >>logs/serverstart.log 2>&1
        mv -f ./OpenComputersMod*lua* ./DELETEME >>logs/serverstart.log 2>&1
...
if [[ "$MC_SERVER_JAVA_PATH" == "DISABLE" ]]; then
    ...
    which $MC_SERVER_JAVA_PATH/java >/dev/null
    if [[ $? == "0" ]]; then
...
echo "DEBUG: Dumping starting variables: " >>logs/serverstart.log
echo "DEBUG: MC_SERVER_MAX_RAM=$MC_SERVER_MAX_RAM" >>logs/serverstart.log
echo "DEBUG: MC_SERVER_GAME_ARGS=$MC_SERVER_GAME_ARGS" >>logs/serverstart.log
echo "DEBUG: MC_SERVER_JAVA_ARGS=$MC_SERVER_JAVA_ARGS" >>logs/serverstart.log
echo "DEBUG: MC_SERVER_CRASH_COUNT=$MC_SERVER_CRASH_COUNT" >>logs/serverstart.log
echo "DEBUG: MC_SERVER_CRASH_TIMER=$MC_SERVER_CRASH_TIMER" >>logs/serverstart.log
echo "DEBUG: MC_SERVER_JAVA_PATH=$MC_SERVER_JAVA_PATH" >>logs/serverstart.log
echo "DEBUG: MC_SERVER_RUN_FROM_BAD_FOLDER=$MC_SERVER_RUN_FROM_BAD_FOLDER" >>logs/serverstart.log
echo "DEBUG: MC_SERVER_IGNORE_OFFLINE=$MC_SERVER_IGNORE_OFFLINE" >>logs/serverstart.log
echo "DEBUG: MC_SERVER_IGNORE_JAVA_CHECK=$MC_SERVER_IGNORE_JAVA_CHECK" >>logs/serverstart.log
echo "DEBUG: MC_SERVER_USE_SPONGE=$MC_SERVER_USE_SPONGE" >>logs/serverstart.log
echo "DEBUG: MC_SERVER_HIGH_CPU_PRIORITY=$MC_SERVER_HIGH_CPU_PRIORITY" >>logs/serverstart.log
echo "DEBUG: MC_SERVER_JMX_ENABLE=$MC_SERVER_JMX_ENABLE" >>logs/serverstart.log
echo "DEBUG: MC_SERVER_JMX_PORT=$MC_SERVER_JMX_PORT" >>logs/serverstart.log
echo "DEBUG: MC_SERVER_JMX_HOST=$MC_SERVER_JMX_HOST" >>logs/serverstart.log
echo "DEBUG: MC_SERVER_MODPACK_NAME=$MC_SERVER_MODPACK_NAME" >>logs/serverstart.log
echo "DEBUG: MC_SERVER_DEFAULT_WORLD_TYPE=$MMC_SERVER_DEFAULT_WORLD_TYPE" >>logs/serverstart.log
echo "DEBUG: MC_SERVER_MCVER=$MC_SERVER_MCVER" >>logs/serverstart.log
echo "DEBUG: MC_SERVER_FORGEVER=$MC_SERVER_FORGEVER" >>logs/serverstart.log
echo "DEBUG: MC_SERVER_FORGESHORT=$MC_SERVER_FORGESHORT" >>logs/serverstart.log
echo "DEBUG: MC_SERVER_FORGEURL=$MC_SERVER_FORGEURL" >>logs/serverstart.log
echo "DEBUG: MC_SERVER_AUTO_RESPOND=$MC_SERVER_AUTO_RESPOND" >>logs/serverstart.log
echo "DEBUG: MC_SERVER_INSTALL_ONLY=$MC_SERVER_INSTALL_ONLY" >>logs/serverstart.log
echo "DEBUG: Basic System Info: " $(uname -a) >>logs/serverstart.log

if [[ $MC_SERVER_environment == "Mac" ]]; then
    echo "DEBUG: Total RAM estimate: " $(sysctl hw.memsize | awk 'BEGIN {total = 1} {if (NR == 1 || NR == 3) total *=$NF} END {print total / 1024 / 1024" MB"}') >>logs/serverstart.log
else
    echo "DEBUG: Total RAM estimate: " $(getconf -a | grep PAGES | awk 'BEGIN {total = 1} {if (NR == 1 || NR == 3) total *=$NF} END {print total / 1024 / 1024" MB"}') >>logs/serverstart.log
fi

echo "DEBUG: Java Version info: "  >>logs/serverstart.log 2>&1
echo "DEBUG: Java path: $MC_SERVER_JAVA" >>logs/serverstart.log 2>&1
$MC_SERVER_JAVA -version >>logs/serverstart.log 2>&1
echo "DEBUG: Dumping current directory listing " >>logs/serverstart.log 2>&1
ls -s1h >>logs/serverstart.log
...
if [[ ${MC_SERVER_JMX_ENABLE} == 1 ]]; then
    ...
    export MC_SERVER_ARGS="${MC_SERVER_JAVA_ARGS} -Dcom.sun.management.jmxremote -Dcom.sun.management.jmxremote.port=${MC_SERVER_JMX_PORT} -Dcom.sun.management.jmxremote.rmi.port=${MC_SERVER_JMX_PORT} -Dcom.sun.management.jmxremote.local.only=false -Dcom.sun.management.jmxremote.authenticate=false -Dcom.sun.management.jmxremote.ssl=false -Djava.rmi.server.hostname=$MS_SERVER_JMX_HOST"
else
...
while $run ; do
    ...
    if [[ ${MC_SERVER_IGNORE_JAVA_CHECK} == 1 ]]; then
        ...
        which $MC_SERVER_JAVA >> /dev/null 2>&1
        if [ $? -eq 0 ]; then
            echo "DEBUG: Java found" >>logs/serverstart.log
            if [[ "$(${MC_SERVER_JAVA} -version 2>&1 | awk -F ' ' '/Bit/ {print $3}')"  == "64-Bit" ]]; then
    ...
    if [[ $? != 0 ]] ; then
        ...
        echo "DEBUG: Found forge jar: ${forge[0]}" >>logs/serverstart.log
    fi
    ...
    if [ ! -f server.properties ]; then
        ...
        echo "level-type=$MC_SERVER_DEFAULT_WORLD_TYPE" >> server.properties
        echo "snooper-enabled=false" >> server.properties
        echo "max-tick-time=90000" >> server.properties
        echo "motd=$MC_SERVER_MODPACK_NAME" >> server.properties
    fi
